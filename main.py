...
async def start(update, context):
    if update.message.chat.type in ['group', 'supergroup']: group_ids.add(update.message.chat.id)
...
async def track_members(update, context):
    if update.message.new_chat_members:
        ...
        for m in update.message.new_chat_members:
            txt = f"🎀 ကြိုဆိုပါတယ် {m.full_name}!\nဆက်သွယ်ရန်: @{TARGET_BOT}"
            await update.message.reply_text(txt)
    if update.message.left_chat_member:
        await update.message.reply_text(f"🥀 နှုတ်ဆက်ပါရစေ {update.message.left_chat_member.full_name}...")
